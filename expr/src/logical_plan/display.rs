...
use panda_common::display::GraphvizBuilder;
use panda_common::tree_node::{TreeNodeVisitor, VisitRecursion};
use panda_common::DataFusionError;
use std::fmt;
...
impl<'a, 'b> TreeNodeVisitor for IndentVisitor<'a, 'b> {
    ...
    fn pre_visit(
        &mut self,
        plan: &LogicalPlan,
    ) -> panda_common::Result<VisitRecursion> {
        if self.indent > 0 {
            writeln!(self.f)?;
        }
...
/// # use panda_expr::logical_plan::display_schema;
