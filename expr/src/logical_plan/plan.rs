...
pub use panda_common::display::{PlanType, StringifiedPlan, ToStringifiedPlan};
pub use panda_common::{JoinConstraint, JoinType};
...
impl LogicalPlan {
    ...
    pub(crate) fn apply_subqueries<F>(&self, op: &mut F) -> panda_common::Result<()>
    where
        F: FnMut(&Self) -> panda_common::Result<VisitRecursion>,
    {
    ...
    pub(crate) fn visit_subqueries<V>(&self, v: &mut V) -> panda_common::Result<()>
    where
...
impl LogicalPlan {
    ...
    /// use panda_expr::{lit, col, LogicalPlanBuilder, logical_plan::table_scan};
...
mod tests {
    ...
    use panda_common::tree_node::TreeNodeVisitor;
    use panda_common::{DFSchema, TableReference};
