...
use panda_common::parsers::CompressionTypeVariant;
use panda_common::{
...
use panda_expr::expr_rewriter::normalize_col_with_schemas_and_ambiguity_check;
use panda_expr::logical_plan::builder::project;
use panda_expr::logical_plan::DdlStatement;
use panda_expr::utils::expr_to_columns;
use panda_expr::{
...
use panda_expr::expr::Placeholder;
...
impl<'a, S: ContextProvider> SqlToRel<'a, S> {
    ...
    fn build_order_by(
        ...
        schema: &DFSchemaRef,
        planner_context: &mut PlannerContext,
    ) -> Result<Vec<Vec<panda_expr::Expr>>> {
    ...
    fn update_to_plan(
        ...
        for (col_name, expr) in values.into_iter() {
            ...
            let expr = match expr {
                panda_expr::Expr::Placeholder(Placeholder {
                    ref id,
                    ref data_type,
                }) => match data_type {
                    None => {
                        let dt = table_schema.data_type(&Column::from_name(&col_name))?;
                        panda_expr::Expr::Placeholder(Placeholder::new(
                            id.clone(),
                            Some(dt.clone()),
                        ))
                    }
    ...
    fn insert_to_plan(
                ...
                .map(|(i, c)| {
                    ...
                    if mapping[column_index].is_some() {
                        return Err(DataFusionError::SchemaError(
                            panda_common::SchemaError::DuplicateUnqualifiedField {
                                name: c,
                            },
                        ));
                    } else {
        ...
        let exprs = index_mapping
            ...
            .map(|i| {
                ...
                let expr =
                    panda_expr::Expr::Column(source_field.unqualified_column())
                        .cast_to(target_field.data_type(), source.schema())?
                        .alias(target_field.name());
                Ok(expr)
            })
            .collect::<Result<Vec<panda_expr::Expr>>>()?;
        let source = project(source, exprs)?;
